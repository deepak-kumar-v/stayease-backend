 ...
 *
 * ownerId/managerId are user ids. We can't validate "this user exists" with a
 * simple annotation (that needs a DB lookup), so the SERVICE checks it and
 * throws if the owner/manager is missing. Annotations here cover the cheap,
 * format-level rules only.
 *
 * checkInTime/checkOutTime accept JSON strings like "14:00" or "11:00:00".
 */
