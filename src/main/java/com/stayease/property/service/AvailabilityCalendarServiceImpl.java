 ...
 *
 * Depends on PropertyService to confirm the property exists (same cross-module
 * pattern as PropertyServiceImpl -> UserService), and enforces one row per
 * (property, date).
 */
@Service
@Transactional
public class AvailabilityCalendarServiceImpl implements AvailabilityCalendarService {
    ...
    public AvailabilityCalendarResponse update(Long id, AvailabilityCalendarRequest request) {
        ...
        // If the date is changing, make sure the new date isn't already taken
        // by a DIFFERENT row of the same property.
        repository.findByPropertyIdAndCalendarDate(request.propertyId(), request.calendarDate())
